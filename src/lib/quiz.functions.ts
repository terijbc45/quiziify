...
export const generateQuestions = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data }) => {
    ...
    const curriculumBlock = data.curriculumContext
      ? `\n\nCURRICULUM GROUND TRUTH — Nepal CDC / NEB VERIFIED TEXTBOOK CONTENT ONLY.\nCountry: ${data.country ?? "Nepal"} | Grade: ${data.grade ?? ""} | Subject: ${data.subject ?? ""}${data.chapter ? ` | Chapter: ${data.chapter}` : ""}\nWEB EVIDENCE (from CDC Sanothimi / NEB / edusanjal / official textbooks):\n${data.curriculumContext}\n\nSTRICT RULES:\n(1) Every question MUST be grounded in the actual chapter content, definitions, formulas, examples, or exercises found in the official Nepal Curriculum Development Centre (CDC) or NEB approved textbook for this exact class + subject + chapter. Do NOT invent facts outside the textbook scope.\n(2) Use vocabulary, notation, and units the textbook itself uses (Nepali translation of technical terms may appear in the explanation).\n(3) Include at least ONE question drawn from a typical exercise / worked example of this chapter.\n(4) Avoid foreign-syllabus content (Indian NCERT, US, IB, Cambridge) unless the CDC/NEB text uses the same material.`
      : "";

    const sysPrompt = `You generate sharp, brain-stimulating ${data.curriculumContext ? "Nepal CDC/NEB school-curriculum" : "general-knowledge"} MCQs designed to make Nepali students master the exact content of their textbooks. Difficulty: ${data.difficulty}.${levelHint} Topic: ${data.topic === "any" ? "ALL fields — mix widely across science, history, geography, math, language, arts, tech, sports, pop-culture, current affairs, biology, chemistry, physics, world cultures" : data.topic}.${avoidHint} For this batch, lean into: ${angle}. Be unpredictable — never recycle classic textbook examples. Variation seed (do NOT mention): ${seed}.${latestBlock}${curriculumBlock}\n\nFORMAT RULES (strict):\n- Default question length: SHORT and clear (one sentence, ~60-130 chars). Only go longer (up to ~220 chars) when extra context is genuinely needed — e.g. specific scientific research, experiments, historic events, or technical questions that require setup for clarity.\n- Each of the 4 options MUST be very short — 1-3 words ideally, NEVER more than 5 words. No full sentences in options.\n- Provide ONE emoji visually representing the subject and a one-sentence explanation.\n- Be accurate, varied, and creative.`;
